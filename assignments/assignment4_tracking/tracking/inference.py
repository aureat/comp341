    ...
    def elapseTime(self, gameState):
        ...
        for oldPosition in self.allPositions:
            ...
            for newPosition, probability in newPositionDist.items():
                newBeliefs[newPosition] += self.beliefs[oldPosition] * probability
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        for i in range(numParticles):
            self.particles.append(legalPositions[i % numPositions])
    ...
    def observeUpdate(self, observation, gameState):
        ...
        # If all particles receive zero weight, reinitialize particles uniformly
        if weights.total() == 0:
            self.initializeUniformly(gameState)
        else:
            weights.normalize()
    ...
    def elapseTime(self, gameState):
        ...
        newParticles = []
        for particle in self.particles:
            newPositionDist = self.getPositionDistribution(gameState, particle)
            newParticles.append(newPositionDist.sample())
        self.particles = newParticles
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        # Generate all possible combinations of ghost positions (Cartesian product)
        allPossiblePositions = list(itertools.product(legalPositions, repeat=self.numGhosts))

        # Shuffle the list to ensure uniform distribution
        random.shuffle(allPossiblePositions)

        # Distribute particles evenly across the shuffled list of possible positions
        numPossiblePositions = len(allPossiblePositions)
        for i in range(self.numParticles):
            position = allPossiblePositions[i % numPossiblePositions]
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for particle in self.particles:
            ...
            for i in range(self.numGhosts):
                ...
                particleWeight *= self.getObservationProb(observation[i], pacmanPosition, ghostPosition, jailPosition)
            weights[particle] += particleWeight

        # If all particles receive zero weight, reinitialize uniformly
        if weights.total() == 0:
            self.initializeUniformly(gameState)
        else:
            weights.normalize()
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            for i in range(self.numGhosts):
                ...
                newParticle[i] = newPositionDist.sample()
